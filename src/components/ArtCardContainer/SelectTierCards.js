...
const SelectTierCards = ({ darkMode }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await axios.get(`/api/tier/user/${currentUser._id}`);
      // const res = await axios.get(
      //   BaseUrl + `/api/tier/user/${currentUser._id}`
      // );
      const artWorkData = res.data.tiers;
      console.log("res:", res.data.tiers);
  ...
  const handleDetails = (id, intervention_id) => {
    navigate(`/Intervention/${intervention_id}/${id}`);
  };
  const ViewDetails = (e, _id, intervention_id) => {
    e.stopPropagation();
    // console.log(_id);
    navigate(`/Intervention/${intervention_id}/details/${_id}`);
  };
  return (
    <>
      <Box
      ...
      >
        {currentUser && currentUser.role == ROLE.INVERTOR ? (
          <GradientButtonPrimary
            // onClick={() =>
            //     navigate(`/${interventionId}/create-tier`)
            // }
            sx={{
              display: "flex",
